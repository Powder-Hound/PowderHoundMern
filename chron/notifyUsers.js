...
const compileAggregates = (resortId, weatherData) => {
  ...
  for (const dataSource in weatherData) {
    ...
    for (let j = 0; j < range.length; j++) {
      ...
      let getAggregates = aggregateSnowfallForRange(
  ...
  for (let i in resortIds) {
    ...
    for (const dataSource in aggregates[resortId]) {
      ...
      sourceMatch.some((val) => {
        let sum;
        if (
          aggregates[resortId][dataSource]["uom"] != user.alertThreshold.uom
        ) {
          sum = uomConverter(
            val.sum,
            aggregates[resortId][dataSource]["uom"],
            user.alertThreshold.uom
          );
        } else {
          sum = val.sum;
        }
...
let compileUserInfo = (user, aggregates, userThreshold) => {
  ...
  let thresholdChecked = checkThresholds(
    user,
    aggregates,
    thresholdValue,
    userThreshold
...
const getAllResorts = () => {
  try {
    let allAggregates = {};
    return ResortWeatherData.find({}).then((results) => {
      results.forEach((resort) => {
        allAggregates[resort.resortId] = compileAggregates(
          resort.resortId,
          resort.weatherData
        )[resort.resortId];
      });
...
const checkForChanges = (usersToNotify, previousData) => {
  for (let previousUser in previousData) {
    ...
    if (Object.keys(usersToNotify).includes(userId)) {
      ...
      if (
        JSON.stringify(previousData[previousUser].resorts) ===
        JSON.stringify(usersToNotify.resorts)
      ) {
...
const sendUserNotifications = (usersToNotify) => {
  for (let userId in usersToNotify) {
    ...
    user.resorts.preferredResorts
      ? (sumOfPrefResorts = Object.keys(user.resorts?.preferredResorts).length)
      : null;
    let sumOfAnyResorts;
    user.resorts.anyResort
      ? (sumOfAnyResorts = Object.keys(user.resorts?.anyResort).length)
      : null;
    ...
    let prefMessage = `${sumOfPrefResorts} preferred ${
      sumOfPrefResorts > 1 ? "resorts" : "resort"
    } with over ${user.prefThreshold}${user.uom} of snow`;
    let anyMessage = `${sumOfAnyResorts} ${
      sumOfAnyResorts > 1 ? "resorts" : "resort"
    } with over ${user.anyThreshold}${user.uom} of snow`;
    let finalMessage;
    let messageEnd =
      messageEndings[Math.floor(Math.random() * messageEndings.length)];
    if (sumOfPrefResorts && sumOfAnyResorts) {
      finalMessage =
        prefMessage + " and " + anyMessage + " ready to " + messageEnd;
